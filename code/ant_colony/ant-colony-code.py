...
for i in range(iteration):
    ...
    temp_visibility = np.ones((m,n,n))*visibility          #creating a copy of visibility
    ...
    while s < n-1:
      ...
      for k in range(0,m):
        ...
        if T[k] + Cij[int(visited[k,s-1])-1,0]/v + t <= Tm:
            ...
            temp_visibility[k][:,cur_loc] = 0     #making visibility of the current city as zero
            p_feature = np.power(pheromne[cur_loc,:],beta)         #calculating pheromne feature 
            v_feature = np.power(temp_visibility[k][cur_loc,:],alpha)  #calculating visibility feature
            #print(p_feature)
            #print(v_feature)
            ...
            #print(r)
            #print(r)
            exhibit = np.nonzero(cum_prob>r)[0][0]+1       #finding the next city having probability higher then random(r) 
            #print(city)
            
            visited[k,s+1] = exhibit              #adding city to route 

            T[k] = T[k] + Cij[int(visited[k,s])-1,int(visited[k,s+1])-1]/v + t
    ...
    for k in range(m):
        ...
        for j in range(n-1):
            ...
            c = c + Cij[int(route_opt[k,j])-1,int(route_opt[k,j+1])-1]   #calcualting total tour distance
    ...
    best_route = visited[dist_min_loc,:]               #intializing current traversed as best route
    pheromne = (1-e)*pheromne                       #evaporation of pheromne with (1-e)

    for k in range(m):
        for j in range(n-1):
            dt = 1/cost[k]
            pheromne[int(route_opt[k,j])-1,int(route_opt[k,j+1])-1] = pheromne[int(route_opt[k,j])-1,int(route_opt[k,j+1])-1] + dt   
...
print('cost of the best path',int(dist_min_cost[0]) + Cij[int(best_route[-2])-1,0])

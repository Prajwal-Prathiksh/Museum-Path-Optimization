...
class TsplibXMLFileContainer(BaseTsplibFileContainer, XMLFileRead):
    ...
    def process_file_data(self):
        self.dataset_name = self.file_data.getElementsByTagName('name')[0].firstChild.data
        """ self.type = self.file_data.getElementsByTagName('name')[0].firstCild.data """
        self.comment = self.file_data.getElementsByTagName('description')[0].firstChild.data
        ...
        for vert_indx in range(len(vertices)):
            for i in range(1, len(vertices[vert_indx].childNodes),2):
                self.cost_matrix[vert_indx, (i+((i//2)>=(vert_indx)))//2] = float(vertices[vert_indx].childNodes[i]._get_attributes().items()[0][1])
